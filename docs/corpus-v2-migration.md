...
The bundled deck corpus predates the structured visual grammar added with schema v2. All seven decks still validate because the runtime contract remains backward-compatible, and `CardRenderSpec` can resolve their legacy `visual_style`, `visual_content`, and `visual_motif` prose as fallbacks. They are nevertheless **authoring-legacy**: none yet has the full structured visual stack expected of newly authored decks.
...
CI runs the same tool with `--check`; that mode currently verifies that every bundled deck parses, has its core axes/cards, and provides a concrete `detailed_description` for every card. It intentionally does **not** fail merely because enrichment is incomplete—the migration is a staged art-direction project, not a mechanical schema emergency.
...
- **Final Fantasy** and **Ultima Octave** do not use the legacy `rank.visual_content` field, so their rank migration must recover/author both narrative and formal rank contributions from the deck itself rather than mechanically splitting an existing prose field.
...
The audit considers a bundled deck visually enriched when its authored source has:
...
4. **Author rank form.** Make each rank formally legible across families, preserving nonstandard rank systems.
